document.addEventListener('DOMContentLoaded', async () => {
  ...
  const preloadContent = () => {
    // Ensure userRole and contentCache are defined in the scope
    let preloadPages;

    if (userRole.toLowerCase() === "student") {
        preloadPages = ["dashboard", "attendance", "grade", "class", "fees", "profile"];
    } else if (userRole.toLowerCase() === "teacher") {
        preloadPages = ["dashboard", "attendance", "grades", "class", "finance", "profile"];
    } else {
        preloadPages = ["dashboard", "students", "teacher", "classes", "school", "finance", "profile"];
    }
...
// Define navigation buttons for each user role
const navigationButtons = {
    ...
    admin: [
      ...
      { page: "messages", icon: "message-circle", label: "Messages", textClass: "text-gray-600 dark:text-gray-400" },
      { page: "profile", icon: "user-circle", label: "Profile", textClass: "text-gray-600 dark:text-gray-400" }
    ]
  };

  function populateNavigation(userRole) {
    console.log('User Role:', userRole);  // Check if userRole is correct
  
    const navContainer = document.getElementById("nav");
    navContainer.innerHTML = ''; // Clear any existing navigation
  
    // Ensure the user role is valid before proceeding
    if (navigationButtons[userRole]) {
      ...
      navigationButtons[userRole].forEach(button => {
        ...
        // Ensure no extra whitespace is passed in the button textClass
        const validTextClass = button.textClass.trim().replace(/\s+/g, '');  // Remove extra spaces
        if (validTextClass) {
          buttonElement.classList.add(validTextClass);
        }
      ...
      console.error("Invalid user role:", userRole);  // If the role is invalid, log the error
    }
  }
